#!/usr/bin/env bash
# Opt-in campaign in the disposable nested lab; never against the physical host.
set -euo pipefail
[[ $(hostname) == smolbox-nested && $(systemd-detect-virt) == kvm && $EUID != 0 ]] || exit 1
source /etc/profile.d/smolbox-lab.sh
cd /opt/smolbox/source
attempt=${SMOLBOX_EXEC_ATTEMPT:-initial}
[[ $attempt =~ ^[a-z0-9]{1,20}$ ]] || exit 1
root=/home/lab/qualification/exec-$attempt
umask 077
mkdir "$root"
mkdir "$root/objects"
...
cleanup() {
  ...
  sudo bash scripts/lab/candidate-control.sh deadline 300
}
trap cleanup EXIT
sudo bash scripts/lab/candidate-control.sh stop
sudo bash scripts/lab/candidate-control.sh deadline 900
...
nsrun() {
  sudo nsenter -t "$pid" -n runuser -u lab -- env \
    ...
    SMOLBOX_HTTP_PORT=28731 SMOLBOX_EXECUTION_ID="exec-1170-$attempt" SMOLBOX_STORE_PARTITION="exec-1170-$attempt" \
    "$@"
}
nsrun mix test test/extended_runtime --include runtime --warnings-as-errors > "$root/runtime.log" 2>&1
curl -fsS --unix-socket /srv/sbq/run/api.sock http://localhost/api/v1/machines > "$root/after-runtime.json"
sudo bash scripts/lab/candidate-control.sh metrics > "$root/runtime-metrics.txt"
# Start a new bounded worker run only after the suite has verified deletion.
...
(
  ...
  nsrun mix test --warnings-as-errors > "$root/store.log" 2>&1
...
sudo bash scripts/lab/candidate-control.sh metrics > "$root/http-metrics.txt"
cleanup
trap - EXIT
sudo bash scripts/lab/kvm-fds.sh smolbox-qual > "$root/kvm-after.txt"
test ! -s "$root/kvm-after.txt"
echo 'Long foreground and durable background HTTP acceptance passed; default deadline restored.'
