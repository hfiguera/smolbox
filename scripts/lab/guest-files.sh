#!/usr/bin/env bash
# Opt-in guest path and file transfer qualification inside the disposable nested Linux lab.
set -euo pipefail
[[ $(hostname) == smolbox-nested && $(systemd-detect-virt) == kvm && $EUID != 0 ]] || exit 1
source /etc/profile.d/smolbox-lab.sh
cd /opt/smolbox/source
attempt=${SMOLBOX_FILES_ATTEMPT:-initial}
[[ $attempt =~ ^[a-z0-9]{1,20}$ ]] || exit 1
root=/home/lab/qualification/guest-files-$attempt
umask 077
mkdir "$root"
mkdir "$root/objects"
...
export SMOLBOX_EXECUTION_ID="guest-files-$attempt" SMOLBOX_STORE_PARTITION="guest-files-$attempt"
(
...
(
  cd examples/durable_host
  phase prepare mix run scripts/guest_files.exs prepare
  phase resume mix run scripts/guest_files.exs resume
)
...
trap - EXIT
echo 'Guest file acceptance passed; worker stopped.'
